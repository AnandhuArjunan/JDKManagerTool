...
public class ChooseDirectoryController implements Initializable {
	...
	public ChooseDirectoryController() {
	...
	class SyncDirectoryBuilder {
		...
		private List<ReloadableController> reloadableControllers = null;
		...
		public SyncDirectoryBuilder haveToReload(ReloadableViews reloadableViews) {
			this.reloadableControllers.add((ReloadableController)dependencyControllers.get(reloadableViews));
			return this;
		}
		...
		public void startSync() {
			...
			Runnable bgThread = () -> {
				...
				Platform.runLater(() -> {
					reloadableControllers.forEach(ReloadableController::reload);
					hideProgessPane();
					showResultPane();
				});
